        ...
        <Grid size={10} m={'auto'}>
          <div {...getRootProps()} className='dropzone-container'>
            ...
            {isDragActive ? (
              ...
              <Typography variant='body1' sx={{ fontStyle: 'italic', fontSize: '1.5rem' }} className='secondary-text'>
                Drop some files here, or
                <span style={{ color: 'var(--primary-dark)' }}> click</span> to select files
              </Typography>
            )}
          </div>
        </Grid>
        <Stack spacing={2} mb={0} sx={{ width: '83%' }} margin={'auto'}>
          {fileError && <Alert severity='warning'>{`Error: ${fileError || 'Invalid file'}`}</Alert>}

          {/* Upload status */}
          {uploadError && <Alert severity='error'>{`Error: ${uploadError || 'Upload failed'}`}</Alert>}

          {uploadSuccess && (
            <Alert severity='success'>{`File ${selectedFile && selectedFile.name} uploaded successfully!`}</Alert>
          )}
